...
for i in `cat ${STRAND_LIST}`; do
	sh ~/GitCode/MitoImputePrep/scripts/BASH/DATA_PROCESSING/ThousandGenomes_Imputation/ThousandGenomes_imputeFrom_RefPan_testKHAP_uniqueSeqs_local.sh ${REFpanel} ${i} ${MCMC} ${BURN} ${KHAP} ${NE}
	echo "JOB FOR ${i} SUBMITTED FOR ${REFpanel} WITH PARAMETERS:	MCMC LENGTH = ${MCMC} (${BURN} BURN-INS),	K_HAP = ${KHAP},	NE = ${NE}"
done