...
# INFO:
#THIS SCRIPT CREATES THE VERSION 5 OF THE CURRENT REFERENCE PANEL
# VERSION 5 OF THE REFERENCE PANEL HAS THE FOLLOWING QUALITIES:
# MINOR ALLELE FREQUENCY >= 1% (0.01)
# GAPS ARE CONVERTED TO MISSING CHARACTER STATES
# THE MASTER ALIGNMENT USED IS FROM JULY 18 2018
...
then
	...
	python ~/GitCode/MitoImputePrep/scripts/PYTHON/ambiguous2missing.py -i ${ALN} -o ${ALN_AMB} -v -g
fi
...
then
	...
	python ~/GitCode/MitoImputePrep/scripts/PYTHON/fasta2vcf_mtDNA.py -i ${ALN_AMB} -o ${VCF_AMB} -v -g
fi
...
then
	...
	Rscript ~/GitCode/MitoImputePrep/scripts/R/removeLowQuality_cmdline.R ${ALN_AMB} ${HQ_FILE} ${GAPS} ${MISSING}
