...
strands_file = "~/GitCode/MitoImputePrep/scripts/INFORMATION_LISTS/custom_mtDNA_platforms.txt"
WGS_file = "~/GitCode/MitoImputePrep/metadata/HaploGrep_concordance/chrMT_1kg_diploid_haplogrep.txt"
geno_dir = "/Volumes/TimMcInerney/MitoImpute/analyses/Haplogroups/HaploGrep/ReferencePanel_v1_0.01/genotyped/"
imp_dir = "/Volumes/TimMcInerney/MitoImpute/analyses/Haplogroups/HaploGrep/ReferencePanel_v1_0.01/imputed/"
comp_dir = "/Volumes/TimMcInerney/MitoImpute/analyses/Haplogroups/HaploGrep/ReferencePanel_v1_0.01/comparison/"
hg_dir = "/Volumes/TimMcInerney/MitoImpute/analyses/Haplogroups/HaploGrep/ReferencePanel_v1_0.01/per_haplogroup/"
...
strands = read.table(strands_file, header = F, stringsAsFactors = F)
names(strands) = c("chip")
#strands = subset(strands, strands$chip %in% analysed_chips)
analysed_chips = strands$chip
...
write.csv(hgs_freq, paste0(hg_dir, "haplogroup_Haplogrep_frequencies_DLOOP.csv"), row.names = F, quote = F)
...
write.csv(mhgs_freq, paste0(hg_dir, "metahaplogroup_Haplogrep_frequencies_DLOOP.csv"), row.names = F, quote = F)


# CREATE A DATA FRAME WITH THE NUMBER OF ROWS = #CHIPS and NUMBER OF COLUMNS #HAPLOGROUPS+1
imputed_df = data.frame(matrix(nrow = length((analysed_chips)), ncol = length(hgs) + 1))
names(imputed_df) = c("chip", hgs)

typed_df = data.frame(matrix(nrow = length((analysed_chips)), ncol = length(hgs) + 1))
names(typed_df) = c("chip", hgs)

# RUN THE CALCULATIONS
DIR = "/Volumes/TimMcInerney/MitoImpute/analyses/Haplogroups/HaploGrep/ReferencePanel_v1_0.01/comparison/haplogroup/" #_HaploGrep_concordance.csv
...
# RUN THE CALCULATIONS
DIR = "/Volumes/TimMcInerney/MitoImpute/analyses/Haplogroups/HaploGrep/ReferencePanel_v1_0.01/comparison/meta_haplogroup/" #_HaploGrep_concordance.csv
for (chip in 1:length(analysed_chips)) { # FOR EACH CHIP
  message(paste0(chip, " / ", length(analysed_chips), " : ", analysed_chips[chip]))
  #print(dir(conc_dir)[chip])
  imputed_df$chip[chip] = analysed_chips[chip]
