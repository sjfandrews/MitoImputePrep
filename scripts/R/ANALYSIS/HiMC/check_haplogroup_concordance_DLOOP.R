...
chips = read.table("~/GitCode/MitoImputePrep/scripts/INFORMATION_LISTS/custom_mtDNA_platforms.txt")
names(chips) = c("array")
...
for (i in 1:length(chips$array)) { # length(chips$array)
  ...
  # REF PANEL v2 (MAF >= 1%)
  if (file.exists(paste0(DIR, chips$array[i], "/ReferencePanel_v1_0.01/MCMC1/chrMT_1kg_", chips$array[i], "_imputed_MCMC", mcmc,"_info"))) {
    ## READ IN THE INFO SCORE
    info.score <- read_delim(paste0(DIR, chips$array[i], "/ReferencePanel_v1_0.01/MCMC1/chrMT_1kg_", chips$array[i], "_imputed_MCMC", mcmc,"_info"), delim = " ")
    ...
    imputed_1kGP <- generate_snp_data(paste0(DIR, chips$array[i], "/ReferencePanel_v1_0.01/MCMC1/chrMT_1kg_", chips$array[i], "_imputed_MCMC", mcmc, ".map"),
                                      paste0(DIR, chips$array[i], "/ReferencePanel_v1_0.01/MCMC1/chrMT_1kg_", chips$array[i], "_imputed_MCMC", mcmc, ".ped"))
    ...
    typed_1kGP <- generate_snp_data(paste0(DIR, chips$array[i], "/ReferencePanel_v1_0.01/chrMT_1kg_", chips$array[i], ".map"),
                                    paste0(DIR, chips$array[i], "/ReferencePanel_v1_0.01/chrMT_1kg_", chips$array[i], ".ped"))
...
table(hg.df$match.t) / nrow(hg.df)
table(hg.df$match.i) / nrow(hg.df)
