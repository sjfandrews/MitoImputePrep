...
# ./haplogrep classify --in ReferencePanel.vcf.gz --format vcf --out ReferencePanel_haplogroups.txt 
...
## For ploting ternery PCA
dat.tern <- select(mtref_dr, Dim.1, Dim.2, Dim.3)  %>% 
  mutate(Dim.1 = Dim.1 + (min(Dim.1) * -1), 
         Dim.2 = Dim.2 + (min(Dim.2) * -1), 
         Dim.3 = Dim.3 + (min(Dim.3) * -1)) %>% 
  as.matrix() %>% 
...
mtref_pcatern_plot <- ggplot(data=dat.tern, aes(x=PC1, y=PC2, z=PC3, colour = macro)) + 
  scale_color_manual(values = plc) + 
  ggtern::coord_tern() + 
  geom_point(size = 0.2) + 
  theme_bw() + 
  ggtern::theme_showarrows() + 
  ggtern::theme_notitles()  + 
...
ggpubr::ggarrange(
  ggplotGrob(mtref_pcatern_plot), mtref_umap_plot
)
...
```{r, echo = TRUE, eval = TRUE}
# tree plot 
tkg_tree_plot <- ggtree(tkg_tree, layout="circular", branch.length = 'branch.length', aes(color = group), size = 0.25) + 
  theme_tree2() +
...
tkg_test_umap_plot <- ggplot() + 
  geom_point(data = mtref_train, aes(x = UMAP1, y = UMAP2), size = 0.01, colour = "#E4E1E3") + 
  geom_point(data = tkg_test, aes(x = UMAP1, y = UMAP2, colour = macro), size = 0.05) + 
  scale_color_manual(values = joint_hg$plc_values, drop = FALSE) +
  guides(colour = guide_legend(override.aes = list(size=2))) +
  theme_bw() + 
  theme(text = element_text(size=10))
...
mtref_train_umap_plot + geom_point(size = 1) +
  theme(legend.box.margin = margin(0, 0, 0, 12), legend.position = "right", 
        text = element_text(size=10)) + 
  guides(col = guide_legend(title = "Haplogroup", title.position = "top", size = 5))
) %>% 
  ggpubr::as_ggplot(.)
...
## tern plot
prow <- cowplot::plot_grid(
  ggplotGrob(mtref_pcatern_plot + theme(legend.position="none", plot.margin=unit(c(-15,-7,-7,-7), "mm"))),
  mtref_train_umap_plot + theme(legend.position="none") + coord_fixed(),
  tkg_tree_plot + theme(legend.position="none") + theme(plot.margin=unit(c(-7.5,-7.5,-7.5,-10), "mm")),
  tkg_test_umap_plot + theme(legend.position="none") + coord_fixed(), 
  labels = c('A', 'B', 'C', 'D'), label_size = 10
                   )

hg.ps <- cowplot::plot_grid(prow, legend_b, rel_widths = c(3, .75), ncol = 2)

ggsave("/Users/sheaandrews/GitCode/MitoImputePrep/Plots/hg.png", plot = hg.ps, height = 170, width = 170, units = 'mm')
ggsave("/Users/sheaandrews/GitCode/MitoImputePrep/Plots/hg.pdf", plot = hg.ps, height = 170, width = 170, units = 'mm')
ggsave("/Users/sheaandrews/GitCode/MitoImputePrep/Plots/hg.tiff", plot = hg.ps, height = 170, width = 170, units = 'mm')
```
...
Additional visualizations of the PCA and UMAP space 

```{r mtref-plot-pca-other}
## mtRef coloured by super-cluster 
ggplot(mtref_dr, aes(x = Dim.1, y = Dim.2, colour = supclu)) + 
  scale_color_manual(values = plc) + 
  geom_point() + theme_bw()

## mtRef coloured ternery colour scheme from first three PCAs
ggplot(mtref_dr, aes(x = Dim.1, y = Dim.2, colour = rgb)) +
...
```{r mtref-pca-plot-3d}
plot_ly(mtref_dr, x = ~Dim.2, y = ~Dim.1, z = ~Dim.3, color = ~macro, colors = plc,
...
plot_ly(mtref_dr, x = ~Dim.2, y = ~Dim.1, z = ~Dim.3, color = ~supclu, colors = plc,
        type = 'scatter3d', mode = 'markers', marker = list(size = 3), 
        hoverinfo = 'text', text = ~paste('</br> ID: ', Individual, 
                                          '</br> Haplogroup: ', macro,
                                          '</br> PC1: ', round(Dim.1, 2),
...
Comparison of the UMAP function in the `umapr`, `umap` and `uwot` packages. 
