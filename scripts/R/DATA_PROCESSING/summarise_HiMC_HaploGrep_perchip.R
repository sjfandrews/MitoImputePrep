...
summary_df = as_tibble(data.frame(array = strand, mcmc = mcmc, refpan_maf = maf, k_hap = k_hap, imputed = imputed, info_cutoff = cutoff)) %>%
  mutate(n_snps_array                                      = nrow(array_snps),
         ...
         q1_himc_concordance_imputed_cutoff                = quantile(himc_df$HiMC_imputed_match, na.rm = T, probs = 0.25),
         mean_himc_concordance_imputed_cutoff              = mean(himc_df$HiMC_imputed_match, na.rm = T),
         sd_himc_concordance_imputed_cutoff                = sd(himc_df$HiMC_imputed_match, na.rm = T),
         median_himc_concordance_imputed_cutoff            = median(as.numeric(himc_df$HiMC_imputed_match), na.rm = T),
         q3_himc_concordance_imputed_cutoff                = quantile(himc_df$HiMC_imputed_match, na.rm = T, probs = 0.75),
         ...
         q1_himc_concordance_imputed_macro_cutoff          = quantile(himc_df$HiMC_imputed_macro_match, na.rm = T, probs = 0.25),
         mean_himc_concordance_imputed_macro_cutoff        = mean(himc_df$HiMC_imputed_macro_match, na.rm = T),
         sd_himc_concordance_imputed_macro_cutoff          = sd(himc_df$HiMC_imputed_macro_match, na.rm = T),
         median_himc_concordance_imputed_macro_cutoff      = median(as.numeric(himc_df$HiMC_imputed_macro_match), na.rm = T),
         q3_himc_concordance_imputed_macro_cutoff          = quantile(himc_df$HiMC_imputed_macro_match, na.rm = T, probs = 0.75),
         ...
         q1_haplogrep_concordance_imputed_cutoff           = quantile(haplogrep_df$HaploGrep_imputed_match, na.rm = T, probs = 0.25),
         mean_haplogrep_concordance_imputed_cutoff         = mean(haplogrep_df$HaploGrep_imputed_match, na.rm = T),
         sd_haplogrep_concordance_imputed_cutoff           = sd(haplogrep_df$HaploGrep_imputed_match, na.rm = T),
         median_haplogrep_concordance_imputed_cutoff       = median(as.numeric(haplogrep_df$HaploGrep_imputed_match), na.rm = T),
         q3_haplogrep_concordance_imputed_cutoff           = quantile(haplogrep_df$HaploGrep_imputed_match, na.rm = T, probs = 0.75),
         ...
         q1_haplogrep_concordance_imputed_macro_cutoff     = quantile(haplogrep_df$HaploGrep_imputed_macro_match, na.rm = T, probs = 0.25),
         mean_haplogrep_concordance_imputed_macro_cutoff   = mean(haplogrep_df$HaploGrep_imputed_macro_match, na.rm = T),
         sd_haplogrep_concordance_imputed_macro_cutoff     = sd(haplogrep_df$HaploGrep_imputed_macro_match, na.rm = T),
         median_haplogrep_concordance_imputed_macro_cutoff = median(as.numeric(haplogrep_df$HaploGrep_imputed_macro_match), na.rm = T),
         q3_haplogrep_concordance_imputed_macro_cutoff     = quantile(haplogrep_df$HaploGrep_imputed_macro_match, na.rm = T, probs = 0.75),
