...
write.csv(inBoth, "~/GitCode/MitoImputePrep/metadata/ADNI_samples_BOTH.csv", row.names = F, quote = F)
write.table(inBoth$SAMPLE_NUMBER, "~/GitCode/MitoImputePrep/metadata/ADNI_samples_BOTH_WGSlabels.txt", row.names = F, quote = F, col.names = F)#cbind(inBoth$TYPED_LABEL, inBoth$SEX)
